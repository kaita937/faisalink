    ...
    <form action="{{ route('admin.equipment.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        ...
        <div class="mb-4">
            ...
            <select name="id_fasilitas" id="id_fasilitas" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                ...
                <option value="{{ $facility->id_fasilitas }}" {{ (isset($selectedFacilityId) && $selectedFacilityId == $facility->id_fasilitas) || old('id_fasilitas') == $facility->id_fasilitas ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama_perlengkapan" id="nama_perlengkapan" value="{{ old('nama_perlengkapan') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>

        <div class="mb-4">
            <label for="jumlah" class="block text-gray-700 text-sm font-bold mb-2">Jumlah</label>
            <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>
